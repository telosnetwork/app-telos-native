    ...
    state.notifications = [...state.notifications].concat({
        ...
        transaction: {
            transactionId: notification.transaction
                ? notification.transaction.transactionId ?? null
                : null,
        },
        read: false,
...
export const initNotifications = (state) => {
    ...
    if (notifs) {
        ...
        state.successCount = state.notifications.filter(
            (n) => !n.read && n.status === 'success'
        ).length;
        state.errorCount = state.notifications.filter(
            (n) => !n.read && n.status === 'error'
        ).length;
    }
};