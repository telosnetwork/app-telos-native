    ...
    <div id="case-file-action">
        <p>
            ...
            <div v-if="act.name === 'makeoffer'">
                <strong>{{ authorization[0].actor }}</strong> made offer with
                estimate <strong>{{ actionData.estimated_hours }}</strong> hours at
            ...
            <div v-if="act.name === 'startcase'">
                ...
                <strong>{{ actionData.number_days_respondant }}</strong> days to
                respond to claims ({{ action.timestamp }})
            </div>
            ...
            <div v-if="act.name === 'reviewclaim'">
                ...
                <span v-if="actionData.claim_info_needed"
                    >&nbsp;Claimant has {{ actionData.number_days_claimant }} days
                    to provide more information.</span
                >&nbsp;
                <span v-if="actionData.response_info_needed"
                    >&nbsp;Respondant has
                    {{ actionData.number_days_respondant }} days to provide more
                    information.</span
                >
                ({{ action.timestamp }})
            </div>
            <div v-if="act.name === 'updateclaim'">
                ...
                <ipfs-link-2 :hash="actionData.claim_link" /> ({{
                    action.timestamp
                }})
            </div>
            ...
            <div v-if="act.name === 'validatecase'">
                <strong>{{ authorization[0].actor }}</strong> (admin)
                <strong>{{ actionData.proceed ? 'validated' : 'rejected' }}</strong>
