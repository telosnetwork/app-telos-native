  ...
  <div class="case-file-action">
    ...
    <div v-if="act.name === 'assigntocase'">
      <i18n-t keypath="pages.resolve.case_action_assign">
        <template #actor
          ><strong>{{ authorization[0].actor }}</strong></template
        >
        <template #arbToAssign
          ><strong>{{ actionData.arb_to_assign }}</strong></template
        >
      </i18n-t>
      ({{ action.timestamp }})
    </div>
    <div v-if="act.name === 'respond'">
      <i18n-t keypath="pages.resolve.case_action_respond">
        ...
        >
        <template #id
          ><strong>{{ actionData.claim_id }}</strong></template
        >
        <template #link
          ><a :href="createIpfsLink(actionData.response_link)"
            >document</a
          ></template
        >
      </i18n-t>
    ...
    <div v-if="act.name === 'makeoffer'">
      <i18n-t keypath="pages.resolve.case_action_make_offer">
        ...
        <template #rate
          ><strong>{{ actionData.hourly_rate }}</strong></template
        > </i18n-t
      >({{ action.timestamp }})
    </div>
    <div v-if="act.name === 'respondoffer'">
      <i18n-t keypath="pages.resolve.case_action_respond_offer">
        ...
        <template #accept>
          {{
            actionData.accept
              ? $t("pages.resolve.accepted")
              : $t("pages.resolve.rejected")
          }}
        </template>
        <template #offer>
        ...
        <template #arbitrator>{{
          getOffer(parseInt(actionData.offer_id)).arbitrator
        }}</template>
      </i18n-t>
      ({{ action.timestamp }})
    </div>
    <div v-if="act.name === 'startcase'">
      <i18n-t keypath="pages.resolve.case_action_start">
        ...
        >
        <template #days
          ><strong>{{ actionData.number_days_respondant }}</strong></template
        >
      </i18n-t>
      ({{ action.timestamp }})
    </div>
    <div v-if="act.name === 'settleclaim'">
      <i18n-t keypath="pages.resolve.case_action_settle">
        ...
        >
        <template #link
          ><a :href="createIpfsLink(actionData.decision_link)"
            >decision</a
          ></template
        >
      </i18n-t>
    ...
    <div v-if="act.name === 'setruling'">
      <i18n-t keypath="pages.resolve.case_action_set_ruling">
        ...
        <template #link
          ><strong
            ><a :href="createIpfsLink(actionData.case_ruling)">ruled</a></strong
          ></template
        >
      </i18n-t>
      ({{ action.timestamp }})
    ...
    <div v-if="act.name === 'closecase'">
      <i18n-t keypath="pages.resolve.case_action_close">
        ...
        >
        <template #close
          ><strong>{{ $t("pages.resolve.closed_case") }}</strong></template
        > </i18n-t
      >({{ action.timestamp }})
    </div>
  </div>
...
export default {
    ...
    // },
        ...
        getOffer(id) {
            ...
            const offer = this.offers.find((offer) => offer.offer_id === id);
            return offer;
        },
        getOfferSyntax(id) {
